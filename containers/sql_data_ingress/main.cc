...
void RunServer() {
  ...
  fns::FnConfidentialTransform service(
      ...
      ProvideSqlDataIngressFnFactory,
  ...
  LOG(INFO) << "SQL Data Ingress Confidential Transform Server listening on "
            << server_address << "\n";
...
int main(int argc, char** argv) {
  confidential_federated_compute::sql_data_ingress::RunServer();
  return 0;
}
