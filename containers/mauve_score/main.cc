...
void RunServer() {
  ...
  // Initialize the Python interpreter and import the MAUVE library.
  // This must happen on the main thread before any gRPC calls.
  PyRuntimeManager py_manager;
  CHECK_OK(py_manager.ImportLib());

  fns::FnConfidentialTransform service(
      std::make_unique<InstanceSigningKeyHandle>(),
      CreateMauveScoreFnFactoryProvider(),
  ...
  LOG(INFO) << "MAUVE Score Confidential Transform Server listening on "
            << server_address << "\n";
...
int main(int argc, char** argv) {
  confidential_federated_compute::mauve_score::RunServer();
  return 0;
}
