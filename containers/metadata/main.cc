...
void RunServer() {
  ...
  fns::FnConfidentialTransform service(
      std::make_unique<InstanceSigningKeyHandle>(), ProvideMetadataMapFnFactory,
  ...
  LOG(INFO) << "Metadata Confidential Transform Server listening on "
            << server_address << "\n";
  ...
  server->Wait();
   ...
   // confidential_federated_compute::metadata

int main(int argc, char** argv) {
  confidential_federated_compute::metadata::RunServer();
  return 0;
}
