 ...
 public:
  ...
  grpc::Status Stream(grpc::ServerContext* context,
    ...
    if (!token_or.ok()) {
      ...
      free_signing_key_handle(private_key_handle);
    ...
    if (server_config == nullptr) {
      LOG(ERROR) << "Rust create_server_session_config returned null.";
      free_signing_key_handle(private_key_handle);
      return grpc::Status(grpc::StatusCode::INTERNAL,
    ...
    if (!server_session_or.ok()) {
      LOG(ERROR) << "Failed to create ServerSession: "
                 << server_session_or.status();
      return grpc::Status(grpc::StatusCode::INTERNAL,
                          absl::StrCat("ServerSession creation failed: ",
                                       server_session_or.status().ToString()));
    }
    ...
    while (!context->IsCancelled()) {
      ...
      {
        auto pump_or = PumpOutgoingMessages(server_session.get(), stream);
        if (!pump_or.ok()) {
          LOG(WARNING) << "PumpOutgoing failed: " << pump_or.status();
...

      {
        auto pump_or = PumpOutgoingMessages(server_session.get(), stream);
        if (!pump_or.ok()) {
          LOG(WARNING) << "PumpOutgoing (reply) failed: " << pump_or.status();
          break;
        }
      }
    }
...
absl::StatusOr<std::unique_ptr<Server>> CreateServer(
  ...
  if (!server) {
    ...
    return absl::InternalError("gRPC server failed to start");
  }
