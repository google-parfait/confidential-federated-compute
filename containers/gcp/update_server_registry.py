...
def update_registry(digest):
    subject_name, subject_digest, commits, custom_metadata, workflows = provenance_lib.fetch_and_verify(digest)
    ...
    for img in registry.get("images", []):
        if img.get("digest") == digest_val:
            print(f"\n[*] Digest {digest_val} already exists in registry. Skipping.")
            return
    ...
    print(f"\n[*] Appended to {os.path.normpath(REGISTRY_PATH)}")
